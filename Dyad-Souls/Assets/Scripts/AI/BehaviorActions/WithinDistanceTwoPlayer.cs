...
{
    [TaskDescription(
        "Determines if the current agent is within distance of either of two specified players. Returns the closest player if within range."
    )]
    ...
    public class WithinDistanceTwoPlayer : Conditional
    {
        ...
        private bool IsWithinDistance(GameObject player)
        {
            ...
            if (Vector3.SqrMagnitude(direction) < m_SqrMagnitude)
            {
                ...
                {
                    var hitTransform = MovementUtility.LineOfSight(
                        transform,
                        m_Offset.Value,
                        player,
                    ...
                    if (
                        hitTransform != null
                        && MovementUtility.IsAncestor(hitTransform, player.transform)
                    )
                    {
        ...
        public override void OnDrawGizmos()
        {
            ...
            UnityEditor.Handles.DrawWireDisc(
                ...
                m_UsePhysics2D ? Owner.transform.forward : Owner.transform.up,
                m_Magnitude.Value
            );
            ...
            if (
                m_PlayerOne != null
                && m_PlayerOne.Value != null
                && m_PlayerOne.Value.transform != null
            )
            {
                UnityEditor.Handles.color = IsWithinDistance(m_PlayerOne.Value)
                    ? Color.green
                    : Color.red;
                UnityEditor.Handles.DrawLine(
                    Owner.transform.position,
                    m_PlayerOne.Value.transform.position
                );
            }

            if (
                m_PlayerTwo != null
                && m_PlayerTwo.Value != null
                && m_PlayerTwo.Value.transform != null
            )
            {
                UnityEditor.Handles.color = IsWithinDistance(m_PlayerTwo.Value)
                    ? Color.green
                    : Color.red;
                UnityEditor.Handles.DrawLine(
                    Owner.transform.position,
                    m_PlayerTwo.Value.transform.position
                );
            }
